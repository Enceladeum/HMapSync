...
//     transform stream.
//   - Remote (apply): ApplyName(objectIndex, name, hideFc, hideName, hideTitle, hideStatus) calls Moniker.SetCharacterName on a
//     peer's puppet so the peer renders the chosen name (and hide flags). ClearName removes it (on teardown / when the sender clears theirs).
public sealed class MonikerService : IDisposable
{
    ...
    // Mirror of Moniker's carried payload (name + hide-FC + hide-name + hide-title + hide-status flags). HideName is
    // additive (Moniker IPC 2.2), HideTitle additive (2.3), HideStatus additive (2.4): JSON from an older sender lacks
    // a newer flag → defaults false; older receivers ignore it. Must stay a superset of what we both read and write, or
    // the flag is silently dropped in transit (which is exactly the class of bug this mirror keeps closed).
    private sealed class NameData { public string Name = ""; public bool HideFcTag; public bool HideName; public bool HideTitle; public bool HideStatus; }
    ...
    public (string name, bool hideFc, bool hideName, bool hideTitle, bool hideStatus) GetLocalName()
    {
        if (!Available || getLocalName == null) return ("", false, false, false, false);
        try
        {
            var json = getLocalName.InvokeFunc();
            if (string.IsNullOrEmpty(json)) return ("", false, false, false, false);
            var data = JsonConvert.DeserializeObject<NameData>(json);
            return data == null ? ("", false, false, false, false) : (data.Name ?? "", data.HideFcTag, data.HideName, data.HideTitle, data.HideStatus);
        }
        catch (Exception ex) { log.Debug("[HMSync] Moniker GetLocalName failed: " + ex.Message); return ("", false, false, false, false); }
    }
    ...
    public void ApplyName(int objectIndex, string name, bool hideFc, bool hideName, bool hideTitle, bool hideStatus, bool forceRedraw = false)
    {
        if (!Available) return;
        ...
        try
        {
            ...
            // for a clear: only clear when there is genuinely nothing to apply (no name AND no hide flag set).
            if (string.IsNullOrEmpty(name) && !hideFc && !hideName && !hideTitle && !hideStatus) { clearName?.InvokeAction(objectIndex); return; }
            ...
            var json = JsonConvert.SerializeObject(new NameData { Name = name, HideFcTag = hideFc, HideName = hideName, HideTitle = hideTitle, HideStatus = hideStatus });
